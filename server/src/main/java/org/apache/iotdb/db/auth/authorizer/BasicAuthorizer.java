...
import org.apache.iotdb.db.exception.auth.DeleteDefaultAdminException;
import org.apache.iotdb.db.exception.auth.GrantAdminException;
...
import org.apache.iotdb.db.exception.auth.RevokeAdminException;
import org.apache.iotdb.db.exception.auth.RoleExistsException;
import org.apache.iotdb.db.exception.auth.RoleNonExistsException;
import org.apache.iotdb.db.exception.auth.UserExistsException;
import org.apache.iotdb.db.exception.auth.UserHasRoleException;
import org.apache.iotdb.db.exception.auth.UserNonExistsException;
import org.apache.iotdb.db.exception.auth.UserNotHasRoleException;
...
public abstract class BasicAuthorizer implements IAuthorizer, IService {
  ...
  public void deleteUser(String username) throws AuthException {
    if (IoTDBConstant.ADMIN_NAME.equals(username)) {
      throw new DeleteDefaultAdminException();
    }
    if (!userManager.deleteUser(username)) {
      throw new UserNonExistsException(username);
    }
  }
  ...
  public void grantPrivilegeToUser(String username, String path, int privilegeId)
    ...
    if (!userManager.grantPrivilegeToUser(username, newPath, privilegeId)) {
      throw new PrivilegeExistsException(username, privilegeId, newPath);
    }
  }
  ...
  public void revokePrivilegeFromUser(String username, String path, int privilegeId)
      throws AuthException {
    if (IoTDBConstant.ADMIN_NAME.equals(username)) {
      throw new RevokeAdminException();
    }
    String p = path;
    if (!PrivilegeType.isPathRelevant(privilegeId)) {
    ...
    if (!userManager.revokePrivilegeFromUser(username, p, privilegeId)) {
      throw new PrivilegeNonExistsException(username, privilegeId, p);
    }
  }
    ...
    if (!success) {
      ...
      for (String user : users) {
        try {
          ...
          logger.warn(
              "Error encountered when revoking a role {} from user {} after deleting the role, "
                  + "because {}", roleName, user, e);
        }
      }
    }
  }
  ...
  public void grantPrivilegeToRole(String roleName, String path, int privilegeId)
    ...
    roleManager.grantPrivilegeToRole(roleName, p, privilegeId);
  }
  ...
  public void grantRoleToUser(String roleName, String username) throws AuthException {
    ...
    if (success) {
      ...
      throw new UserHasRoleException(username, roleName);
    }
  }
  ...
  public void revokeRoleFromUser(String roleName, String username) throws AuthException {
    ...
    if (!userManager.revokeRoleFromUser(roleName, username)) {
      throw new UserNotHasRoleException(username, roleName);
    }
  }
